...
ECS mesh rendering with depth, directional Lambert lighting, texture/material contracts, UV0, consecutive instancing, CPU frustum AABB culling and a fail-closed static GLB importer (triangle primitives → `MeshData` / optional UV0) are implemented. PBR, shadows, skinning shaders, transparency, post-processing, full glTF scenes/nodes/textures and `.gltf`+URI buffers remain subsequent rendering work. The historical validation-triangle presenter remains available as a separate low-level tool.
