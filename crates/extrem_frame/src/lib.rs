...
//!
//! This crate deliberately does not fabricate GPU timing. GPU time is optional
//! and only participates when a caller has a real measurement source (for
//! example WebGPU timestamp queries). CPU frame time is always explicit.

use core::fmt;
use core::time::Duration;

/// Errors returned by frame-budget construction or sampling.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FrameError {
    /// Target refresh rate must be finite and strictly positive.
    InvalidTargetHz,
    /// A measured duration exceeded the representable nanosecond range.
    DurationOverflow,
}
...
/// Frame target expressed as an exact nanosecond budget.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl FrameBudget {
    /// Build a frame budget from a target refresh rate.
    ///
    /// The rate is quantized to milli-hertz to make the resulting budget stable
    /// and auditable across platforms. For 60 Hz the resulting budget is
    ...
    /// Quantized target refresh rate in milli-hertz.
    #[must_use]
...
/// One observed frame. GPU timing is absent unless the caller measured it.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct FrameSample {
    /// End-to-end CPU/wall frame duration.
    pub cpu_frame: Duration,
    /// GPU execution duration when exposed by the active backend.
    pub gpu_frame: Option<Duration>,
}

/// Budget classification for a frame.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Auditable evaluation of one frame against a target budget.
...
impl FrameBudget {
    /// Evaluate one sample without inventing unavailable GPU data.
    #[must_use]
...
/// Streaming frame-time statistics using Welford's numerically stable update.
#[derive(Debug, Clone, Copy, PartialEq)]
...
impl FrameStats {
    /// Record an observed CPU frame duration against the declared budget.
    ...
    /// Mean CPU frame time in milliseconds.
    #[must_use]
    pub const fn mean_ms(self) -> f64 {
        self.mean_ms
    }

    /// Population standard deviation of CPU frame time in milliseconds.
    #[must_use]
    ...
    /// Fraction of observed frames that exceeded the target budget.
    #[must_use]
...
mod tests {
    ...
    fn rejects_invalid_rates() {
        ...
        assert_eq!(FrameBudget::from_hz(f64::NAN), Err(FrameError::InvalidTargetHz));
    }
