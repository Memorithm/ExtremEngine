use extrem_gpu::{
    MAX_FRAME_DRAWS, MeshData, MeshDraw, MeshError, MeshVertex,
...
fn rejects_empty_incomplete_and_out_of_bounds_geometry() {
    assert_eq!(MeshData::new(Vec::new(), vec![0, 0, 0]).unwrap_err(), MeshError::EmptyGeometry);
    assert_eq!(MeshData::new(vertices(), Vec::new()).unwrap_err(), MeshError::EmptyGeometry);
    assert_eq!(MeshData::new(vertices(), vec![0, 1]).unwrap_err(), MeshError::InvalidIndexCount);
    for index in [3, u32::MAX] {
        assert_eq!(MeshData::new(vertices(), vec![0, 1, index]).unwrap_err(), MeshError::IndexOutOfBounds);
    }
}
...
fn rejects_nonfinite_vertices_and_invalid_linear_colors() {
    for value in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
        ...
        assert_eq!(MeshData::new(data, vec![0, 1, 2]).unwrap_err(), MeshError::InvalidVertex);
    }
...
fn rejects_nonfinite_and_overflowing_camera_model_composition() {
    ...
    assert_eq!(validate_frame(&identity(), &[item]), Err(MeshError::InvalidMatrix));
    let mut item = draw();
    item.model[0] = f32::MAX;
    let mut camera = identity();
    camera[0] = 2.0;
    assert_eq!(validate_frame(&camera, &[item]), Err(MeshError::InvalidMatrix));
...
fn frame_count_and_extent_limits_are_checked_without_gpu_allocations() {
    ...
    assert_eq!(validate_frame(&identity(), &draws), Err(MeshError::Capacity));
